...
public:
    long long maximumProduct(vector<int>& nums, int m) {
       ...
       for ( int i = 0; i < nums.size(); ++i ) {
           long long subseq_maxprod = subseqMaxProd(nums, m, i);
           max = (subseq_maxprod > max) ? subseq_maxprod : max;
       }
        ...
        // auto max_measure = [m](pair<struct ValIdxPair, struct ValIdxPair>& pp)->int {
